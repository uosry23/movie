...
export const getCastMovies = createAsyncThunk(
  "getmoviescast",
  async (id, thunkAPI) => {
...
const castMOvies = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getCastMovies.pending, (state) => {
      state.loadCast = true;
      state.errorCast = null;
      //   console.log(state.castmovie);
    });
    builder.addCase(getCastMovies.fulfilled, (state, action) => {
    ...
    builder.addCase(getCastMovies.rejected, (state, action) => {
      state.loadCast = false;
...
export const castMovies = castMOvies.reducer;
